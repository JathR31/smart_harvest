...
<body class="antialiased flex items-center justify-center py-10 min-h-screen" x-data="{ open: false }">
    ...
    <div class="register-card w-full max-w-xl p-8 bg-white rounded-2xl">
        ...
        <div class="mb-8 p-4 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
            <strong data-translate data-translate-id="note">🚀 Fastest Way to Get Started:</strong> <span>Sign up with Google for instant access - no verification delays!</span>
        </div>
        ...
        <div class="mb-6">
            <a href="{{ route('auth.google') }}" class="w-full py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-xl hover:shadow-lg transition duration-300 shadow-md flex items-center justify-center gap-2 group">
                ...
                <span data-translate data-translate-id="sign-up-google">Sign up with Google</span>
            </a>
            <p class="text-xs text-gray-500 mt-2 text-center">Instant access • No verification required • Secure</p>
        </div>

        <!-- Divider -->
        <div class="flex items-center my-6">
            <div class="flex-1 border-t border-gray-300"></div>
            <span class="px-3 text-sm font-medium text-gray-600">or</span>
            <div class="flex-1 border-t border-gray-300"></div>
        </div>

        <form method="POST" action="{{ route('register.attempt') }}" id="traditional-register">
            @csrf
