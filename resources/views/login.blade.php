...
<body class="antialiased flex items-center justify-center min-h-screen" x-data="{ loginMode: 'email' }">
    ...
    <div class="login-card w-full max-w-sm p-8 bg-white rounded-2xl">
        ...
        <div class="mb-6">
            <div class="grid grid-cols-2 gap-2 rounded-2xl bg-gray-100 p-1">
                <button type="button" @click="loginMode = 'email'" :class="loginMode === 'email' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-green-700'" class="rounded-xl px-3 py-3 text-sm font-semibold transition">
                    Email Login
                </button>
                <button type="button" @click="loginMode = 'rsbsa'" :class="loginMode === 'rsbsa' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-green-700'" class="rounded-xl px-3 py-3 text-sm font-semibold transition">
                    RSBSA Login
                </button>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('login.attempt') }}">
            ...
            <!-- Email / RSBSA Field -->
            <div class="mb-4">
                <label for="email" class="block text-sm font-medium text-gray-700 mb-2" x-text="loginMode === 'rsbsa' ? 'RSBSA Number' : 'Email'"></label>
                <div class="relative">
                    <input id="email" type="text" name="email" required autofocus
                        ...
                        :placeholder="loginMode === 'rsbsa' ? '4-11-10-001-00045' : 'farmer@example.com'" value="{{ old('email') }}">
                ...
                <p class="text-xs text-gray-500 mt-1 pl-1" x-text="loginMode === 'rsbsa' ? 'Use your RSBSA number to sign in without a password.' : 'Use your email address and password to sign in.'"></p>
            </div>
